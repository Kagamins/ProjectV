...
public partial class PlayerUIManager : MonoBehaviour
{
	...
	*/
	...
	public static float BarSize = Screen.width*0.3f;
...
//	}
	...
	public void OnGUI()
	{
		...
		GUI.Label (new Rect(LFRect.x,LFRect.y,LFRect.width,40),GameInformation.PlayerName,"Box");
		...
		GUI.Box (new Rect(0,Screen.height-Screen.height/8,Screen.width ,Screen.height/8),GameInformation.CurrentXp.ToString()+"/"+GameInformation.RequiredXP.ToString());
		GUI.Box (new Rect(0,Screen.height-Screen.height/8,Screen.width*(GameInformation.CurrentXp/(float)GameInformation.RequiredXP),Screen.height/8)," ");
		GUI.Label (HBrRect,Health+"/"+maxHealth);
//		GUI.Label (SBrRect,Stamina+"/"+MaxStamina);
//		GUI.Label (MBrRect,Mana+"/"+MaxMana);
//
...
//		Health =   Health + Health* GameInformation.Vitality;
		maxHealth =  Health+ Health* GameInformation.Vitality;
//		Mana =   Mana +Mana* GameInformation.Intellect;
		MaxMana = Mana +MaxMana *GameInformation.Intellect;
//		Stamina =  Stamina + Stamina* GameInformation.Endurance;
		MaxStamina = Stamina+ Stamina * GameInformation.Endurance;
 	}
...
//			}GUILayout.EndHorizontal ();}
	...
	public static void ScaleAdjustment(){
		CrosshairRect = new Rect (Screen.width / 2 - CrosshairSize / 2, Screen.height / 2 - CrosshairSize / 2, CrosshairSize, CrosshairSize);
		HBrRect = new Rect(Screen.width/75,Screen.height/130, BarSize* (Health/(float)maxHealth),BarSize/3);
		MBrRect = new Rect(Screen.width/75,Screen.height/130+BarSize/3, BarSize* (Mana/(float)MaxMana),BarSize/3);
		SBrRect = new Rect(Screen.width/75,Screen.height/130+BarSize/3*2, BarSize* (Stamina/(float)MaxStamina),BarSize/3);
		BG1 = new Rect(Screen.width/75,Screen.height/130, BarSize,BarSize/3);
		BG2 = new Rect(Screen.width/75,Screen.height/130+BarSize/3, BarSize,BarSize/3);
		BG3 = new Rect(Screen.width/75,Screen.height/130+BarSize/3*2, BarSize,BarSize/3);
		LFRect = new Rect (Screen.width/2-Screen.width/2,Screen.height/2+Screen.height/4,Screen.width/6,Screen.height/5);
	}
}